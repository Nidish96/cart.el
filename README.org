...
2. Support for modifying existing objects.
3. Making node information for each point possible as an option (instead of having to click on successive points repeatedly and have common node properties).
