...
_*UPDATE VER 0.2:*_ Cart.el now supports a basic Graphical Front-End based on =artist-mode=!
...
4. Some mechanism simulating graphical feedback.
   Two ideas for this:
   1. Must experiment with *spawning a transparent frame* spanning the whole screen until the user input is done.
      Here's elisp code to create a transparent frame without modeline & minibuffer.
      Perhaps if we can invoke some emacs-native WYSIWYG drawing interface while the user clicks, this could be a solution.
      How expensive this will be is up for discussion.
      ...
      #+begin_src emacs-lisp :results none
        ...
        (artist-clear-buffer (current-buffer))
        (setq-local mode-line-format nil)

        (setq ev (read-event))
        (artist-down-mouse-1 ev)
        ;; (setq xys (cart--gmc))
        ;; (setq x1 (elt (elt xys 0) 0))
        ;; (setq y1 (elt (elt xys 0) 1))
        ;; (setq x2 (elt (elt xys 1) 0))
        ;; (setq y2 (elt (elt xys 1) 1))
