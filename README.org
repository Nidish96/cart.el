...
** Quick Bugfixes
1. In =cart-tikz-draw= when tangents have been specified, the path is not closed smoothly when user says yes to close curve.
2. =cart-translate-tikz= hangs when there is text where we're selecting. Need to disable all other click events during this.
